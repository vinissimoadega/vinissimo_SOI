...
import type { SaleDetail, SaleListResponse } from "@/types";
...
export async function getSales(query: URLSearchParams): Promise<SaleListResponse> {
  const suffix = query.toString() ? `?${query.toString()}` : "";
  return serverApiFetch<SaleListResponse>(`/sales${suffix}`);
}

export async function getSale(saleId: string): Promise<SaleDetail> {
  return serverApiFetch<SaleDetail>(`/sales/${saleId}`);
}
