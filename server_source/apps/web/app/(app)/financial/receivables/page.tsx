...
import { ReceivableActions } from "@/components/financial/financial-actions";
...
import { getFinancialReceivables } from "@/lib/financial";
import type { FinancialReceivableItem } from "@/types";
...
function buildQuery(searchParams?: PageProps["searchParams"]) {
  ...
  return {
    query,
    filters: { search, channelId, status, dateFrom, dateTo, page, pageSize },
  };
}
...
function buildPageHref(
  ...
  return `/financial/receivables?${query.toString()}`;
}
...
export default async function FinancialReceivablesPage({
  ...
  const response = await getFinancialReceivables(query);
  ...
  return (
    <div className="page-shell">
      <header className="page-header">
        <div>
          ...
          <p className="mt-2 text-sm text-black/55">
            Recebíveis gerados pela operação, separados entre previsto, vencido e realizado.
          </p>
        </div>
      ...
      <SectionCard
        title="Filtros"
        subtitle="Refine por canal, status, período e texto operacional."
      >
        <form className="grid gap-3 md:grid-cols-6" method="get">
          <input
            ...
            name="search"
            placeholder="Buscar por título, venda, cliente ou canal"
          />
          <select
      ...
      <SectionCard
        ...
        subtitle={`${response.pagination.totalItems} conta(s) a receber nesta leitura.`}
      >
          ...
          columns={[
            {
              ...
              render: (row) => (
                <div className="space-y-1">
                  <p className="font-semibold text-grafite">{row.receivableNumber}</p>
                  <p className="text-xs text-black/45">
                    {row.saleNumber ? `Venda ${row.saleNumber}` : row.sourceLabel}
                  </p>
                </div>
              ),
            },
            {
              ...
              render: (row) => (
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-black/45">{row.channelName ?? "Sem canal"}</p>
                </div>
              ),
            },
            {
              ...
              render: (row) => (
                <div className="space-y-1 text-sm">
                  <p>Bruto: {formatCurrency(row.grossAmount)}</p>
                  <p>Líquido: {formatCurrency(row.netExpectedAmount)}</p>
                  <p>Recebido: {formatCurrency(row.amountReceived)}</p>
                </div>
              ),
            },
            {
            ...
            },
            {
              key: "status",
              label: "Status",
              render: (row) => <FinancialReceivableStatusBadge status={row.status} />,
            },
            {
              key: "actions",
              label: "Ações",
              render: (row) => <ReceivableActions receivable={row} />,
              className: "w-full md:w-[280px]",
            },
          ]}
          emptyMessage="Nenhuma conta a receber encontrada com os filtros atuais."
          rows={response.items}
        />
