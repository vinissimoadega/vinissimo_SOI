...
type CustomerMetricsAggregateRow = {
...
export class SalesService {
  ...
  async listSales(filters: SaleListFilters) {
    ...
    const itemsResult = await this.db.query<SaleRow>(
      `${this.buildSaleProjectionQuery(whereClause)}
  ...
  async getSaleById(saleId: string): Promise<SaleDetailRecord> {
    const saleResult = await this.db.query<SaleRow>(
      `${this.buildSaleProjectionQuery('WHERE so.id = $1')}
       LIMIT 1;`,
      [saleId],
    );
  ...
  private buildSaleProjectionQuery(whereClause: string) {
    return `SELECT
         so.id,
  ...
  async createSale(
    ...
    return this.db.withClient(async (client) => {
      ...
      try {
        ...
        await this.financialService.syncReceivableForSale(
          client,
          saleId,
          currentUser,
        );

        await this.syncCustomerMetricsForCustomers(client, [payload.customerId]);
  ...
  async updateSale(
    ...
    return this.db.withClient(async (client) => {
      ...
      try {
        ...
        await this.syncCustomerMetricsForCustomers(client, [
          existingSale.customerId,
          patch.customerId ?? existingSale.customerId,
        ]);
  ...
  private async syncCustomerMetricsForCustomers(
    client: PoolClient,
    customerIds: Array<string | null | undefined>,
  ) {
    const ids = [...new Set(customerIds.filter((value): value is string => !!value))];

    if (ids.length === 0) {
      return;
    }

    const settingsResult = await client.query<{ customerInactiveDays: string | null }>(
      `SELECT
         ss.customer_inactive_days::text AS "customerInactiveDays"
       FROM soi.system_settings AS ss
       WHERE ss.is_current = true
       ORDER BY ss.effective_from DESC
       LIMIT 1;`,
    );
    const inactiveDays = Number(settingsResult.rows[0]?.customerInactiveDays ?? '45');
    const normalizedInactiveDays =
      Number.isFinite(inactiveDays) && inactiveDays > 0 ? inactiveDays : 45;

    for (const customerId of ids) {
      const aggregateResult = await client.query<CustomerMetricsAggregateRow>(
        `SELECT
           MIN(so.sale_date) AS "firstPurchaseAt",
           MAX(so.sale_date) AS "lastPurchaseAt",
           COUNT(*)::text AS "ordersCount",
           COALESCE(SUM(COALESCE(so.net_revenue, 0)), 0)::text AS "totalRevenue"
         FROM soi.sales_orders AS so
      ...
      const avgTicket =
        ordersCount > 0 ? (totalRevenue / ordersCount).toFixed(2) : null;
      const customerStatus = this.deriveCustomerStatus(
      ...
      await client.query(
        `INSERT INTO soi.customer_metrics (
         ...
         SET first_purchase_at = EXCLUDED.first_purchase_at,
  ...
  private deriveCustomerStatus(
    ...
    if (lastPurchaseAt) {
      ...
      if (
        ...
        purchaseDate < inactivityThreshold
      ) {
        return 'inativo';
      }
    }
