...
export function WhatsappQuickOrder({
  ...
  const [items, setItems] = useState<QuickOrderItemDraft[]>([
    buildItemDraft(availableProducts),
  ]);
  ...
  async function handleDetectedSku(sku: string) {
    ...
    if (!targetRowId) {
      setError("Escolha a linha do pedido antes de usar a câmera.");
      return;
    }
    ...
    if (!lookup.found || !lookup.product) {
      setError(
        `O SKU ${lookup.sku} ainda não está cadastrado. Cadastre o produto para continuar o pedido.`,
      );
    ...
    setMessage(
      `Produto localizado pela câmera: ${lookup.product.sku} · ${lookup.product.name}. Item do pedido preenchido automaticamente.`,
    );
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="space-y-3">
            {items.map((item) => (
              <div
                key={item.rowId}
                className="grid gap-3 rounded-2xl border border-black/10 bg-black/[0.02] p-4 lg:grid-cols-[2fr_1fr_1fr_auto]"
              >
                <select
                  className="input-soft"
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            <button className="btn-secondary w-full sm:w-auto" type="button" onClick={addItem}>
              Adicionar item
            </button>
          ...
          >
            <div className="space-y-4">
              {additionalCosts.map((cost) => (
                <div
                  key={cost.rowId}
                  className="grid gap-3 rounded-2xl border border-black/10 bg-black/[0.02] p-4 lg:grid-cols-[1.4fr_1.6fr_1fr_auto]"
                >
                  <select
                    className="input-soft"
                    value={cost.costType}
              ...
              <div className="flex flex-col gap-3 rounded-2xl border border-dashed border-black/15 bg-black/[0.03] px-4 py-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                <button
